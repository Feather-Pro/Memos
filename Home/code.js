...
async function __memes__(type) {
    const response = await fetch(`https://${port}/api-key32/${type}/count`)
    const count = await response.json()
    let group = ''
    for (var id = count;id > 0;id --) {
        const memeResponse = await fetch(`https://${port}/api-key32/${type}/${id}`)
...
async function searchResults(type , event) {
    ...
    const response = await fetch(`https://${port}/api-key32/${memeType}/count`)
    const count = await response.json()
    let results = []
    for (var id = count;id > 0;id --) {
        const memeResponse = await fetch(`https://${port}/api-key32/${memeType}/${id}`)
        const data = await memeResponse.json()
    ...
    for (var id = 0;id < results.length;id ++) {
        const memeResponse = await fetch(`https://${port}/api-key32/${memeType}/${results[id]}`)
        const data = await memeResponse.json()
        ...
        group += `<a href="https://${port}/${memeType}/${data.id}?i=${id}&t=${theme[0]}" target="_blank"><figure>${extention}<figcaption>${data.name}</figcaption></figure></a>`
    }
